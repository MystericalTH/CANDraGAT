...
def main():
    ...
    mainlogger.info('Start time:',start_time_formatted)
    os.makedirs(RUN_DIR)
    ...
    trainlogger = MyLogging.getLogger('pbar.train', filename=f'{RUN_DIR}/train.log')
    for fold, (Trainset, Validset) in tqdm.tqdm(enumerate(df_kfold_split(drugsens_tv),start=0), total=5, file = TqdmToLogger(trainlogger,level=logging.INFO)): 
        trainlogger.info(f'\n=============== Fold {fold+1}/{folds} ===============\n')

        seed = set_seed(100)
        trainlogger.info(f'-- TRAIN SET {fold+1} --')
        ...
        trainlogger.info("Start Training...")
        stop_flag = False
        ...
        for num_epoch in range(1,max_epoch+1):
            ...
            cum_loss = 0.0
            printloss = 0.0
            trainlogger.info(f'Epoch:{num_epoch}/{max_epoch}')
            status.update({
                    'fold':fold,
                    'epoch':num_epoch
                })
            ...
            for ii, Data in tqdm.tqdm(enumerate(trainloader), total=len(trainloader), file = TqdmToLogger(trainlogger,level=logging.INFO)): 
                ...
                if (ii+1) % printrate == 0:
                    ...
                    trainlogger.info("Iteration {}/{}".format(ii+1, len(trainloader)))
                    trainlogger.info(f"Duration = {duration_iter} seconds, loss = {printloss/printrate:.4f}")
                    printloss = 0.0
                    start_iter = datetime.now()
            ...
            trainlogger.info(f"Epoch duration = {duration_epoch} seconds")
            trainlogger.info(f"Train loss on Epoch {num_epoch} = {trainmeanloss}")

...
class AttentionCalculator(nn.Module):
    ...
    def forward(self, FP_align, atom_neighbor_list):
        ...
        attend_mask = attend_mask.type(torch.FloatTensor).unsqueeze(-1).to(self._device)
        ...
        softmax_mask = softmax_mask.type(torch.FloatTensor).unsqueeze(-1).to(self._device)
...
class FPInitializer(nn.Module):
    ...
    def forward(self, atom_features, bond_features, atom_neighbor_list, bond_neighbor_list):
        ...
        neighbor_FP = torch.cat([atom_neighbor_features, bond_neighbor_features], dim=-1)
...
class AttentiveFP_mol(nn.Module):
    ...
    def forward(self, atom_FP, atom_mask):
        ...
        mol_softmax_mask = mol_softmax_mask.type(torch.FloatTensor).to(self._device)
        # [batch, atom_length, 1]
